...
float Vector::getAngleDeg() {
    if (dirtyDR) recalcDR();
    // qDebug("getAngleDeg:  %f", _angle*180/M_PI);
    // qDebug("_x %f", _x );
    // qDebug("_y %f", _y );

    float ang = (_angle / M_PI) * 180.0;
    return float(ang);
}
...
void Vector::setAngle(float angle){
    if (dirtyDR) recalcDR();
    _angle=(double(normalize(angle*M_PI/180.0)));
    dirtyXY=true;
}
...
void Vector::recalcDR() {
    ...
    // qDebug("_angle:  %f", _angle*180/M_PI);
